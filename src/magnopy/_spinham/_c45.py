...
def _get_primary_p45(alpha, beta, gamma, epsilon, nu, _lambda, rho, parameter=None):
    r"""
...
def _add_45(
    ...
    alpha, beta, gamma, epsilon, nu, _lambda, rho, parameter = _get_primary_p45(
        alpha=alpha,
        beta=beta,
        gamma=gamma,
        epsilon=epsilon,
    ...
    index = 0
    while index < len(spinham._45):
        # If already present in the model
        if spinham._45[index][:7] == [alpha, beta, gamma, epsilon, nu, _lambda, rho]:
            # Either replace
            if when_present.lower() == "replace":
                spinham._45[index][7] = parameter
            # Or add
            elif when_present.lower() == "add":
                spinham._45[index][7] = spinham._45[index][7] + parameter
            # Or replace with mean value
            elif when_present.lower() == "mean":
                spinham._45[index][7] = (spinham._45[index][7] + parameter) / 2.0
            # Or do nothing
    ...
    spinham._45.append([alpha, beta, gamma, epsilon, nu, _lambda, rho, parameter])
...
def _remove_45(
    ...
    alpha, beta, gamma, epsilon, nu, _lambda, rho = _get_primary_p45(
        alpha=alpha,
        beta=beta,
        gamma=gamma,
        epsilon=epsilon,
    ...
    for index in range(len(spinham._45)):
