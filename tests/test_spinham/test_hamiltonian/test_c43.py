...
def test_add_43(alpha, beta, nu, parameter):
    spinham = get_spinham()

    if 0 <= alpha < len(spinham.atoms.names) and 0 <= beta < len(spinham.atoms.names):
        spinham.add_43(alpha, beta, nu, parameter)
    else:
...
def test_add_43_when_present(when_present, parameter):
    spinham = get_spinham()

    spinham.add_43(alpha=0, beta=1, nu=(1, 0, 0), parameter=np.ones((3, 3, 3, 3)))
    ...
    assert np.allclose(spinham._43[0][3], parameter * np.ones((3, 3, 3, 3)))
...
def test_add_43_sorting(
    ...
    spinham.add_43(alpha1, beta1, nu1, parameter)

    if [alpha1, beta1, nu1] == [alpha2, beta2, nu2]:
        with pytest.raises(ValueError):
            spinham.add_43(alpha2, beta2, nu2, parameter)
    else:
        spinham.add_43(alpha2, beta2, nu2, parameter)

    spinham.add_43(alpha3, beta3, nu3, parameter, when_present="replace")
    spinham.add_43(alpha4, beta4, nu4, parameter, when_present="replace")
...
def test_remove_43(r_alpha, r_beta, r_nu, nus):
...
def test_mul(parameter, number):
    ...
    spinham.add_43(alpha=0, beta=1, nu=(0, 3, -4), parameter=parameter)
    spinham.add_43(alpha=4, beta=2, nu=(1, 0, 0), parameter=parameter * 1.43)
    spinham.add_43(alpha=7, beta=5, nu=(0, 0, 0), parameter=parameter / 3)
...
def test_add(parameter1, parameter2):
    ...
    spinham1.add_43(alpha=0, beta=1, nu=(0, 3, -4), parameter=parameter1)
    spinham1.add_43(alpha=4, beta=2, nu=(1, 0, 0), parameter=parameter1 * 1.32)
    spinham1.add_43(alpha=7, beta=5, nu=(0, 0, 0), parameter=parameter1 / 3)

    spinham2.add_43(alpha=0, beta=1, nu=(0, 3, -4), parameter=parameter2)
    spinham2.add_43(alpha=4, beta=2, nu=(1, 0, 0), parameter=parameter2 * 1.32)
    spinham2.add_43(alpha=8, beta=5, nu=(0, 0, 0), parameter=parameter2 / 3)

    m_spinham = spinham1 + spinham2

    assert m_spinham.M == 7

    assert len(m_spinham.p43) == 8
...
def test_make_supercell(parameter1, i, j, k):
    ...
    spinham.add_43(alpha=0, beta=1, nu=(0, 0, 0), parameter=parameter1)
    spinham.add_43(alpha=0, beta=1, nu=(1, 0, 0), parameter=parameter1 * 1.42)
