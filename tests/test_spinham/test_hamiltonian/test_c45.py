...
from magnopy._spinham._c45 import _get_primary_p45
...
def test_add_45(alpha, beta, gamma, epsilon, nu, _lambda, rho, parameter):
    spinham = get_spinham()
    ...
    if (
        ...
        spinham.add_45(alpha, beta, gamma, epsilon, nu, _lambda, rho, parameter)
    else:
...
def test_add_45_when_present(when_present, parameter):
    spinham = get_spinham()
    ...
    assert np.allclose(spinham._45[0][7], parameter * np.ones((3, 3, 3, 3)))
...
def test_add_45_sorting(
    ...
    spinham.add_45(alpha1, beta1, gamma1, epsilon1, nu1, _lambda1, rho1, parameter)
    ...
    if [alpha1, beta1, gamma1, epsilon1, nu1, _lambda1, rho1] == [
        ...
        spinham.add_45(alpha2, beta2, gamma2, epsilon2, nu2, _lambda2, rho2, parameter)
    ...
    for i in range(len(spinham._45) - 1):
        assert spinham._45[i][:-1] <= spinham._45[i + 1][:-1]
...
def test_remove_45(
    ...
    if (
        ...
        original_bonds = [tmp[:-1] for tmp in spinham._45]
        original_length = len(spinham._45)

        spinham.remove_45(*bond)

        primary_bond = list(_get_primary_p45(*bond))
        if primary_bond in original_bonds:
            updated_bonds = [tmp[:-1] for tmp in spinham._45]

            assert len(spinham._45) == original_length - 1
            assert bond not in updated_bonds
            assert primary_bond not in updated_bonds
        else:
            assert len(spinham._45) == original_length
...
def test_mul(parameter, number):
    ...
    assert len(spinham.p45) == len(m_spinham.p45)

    params = list(spinham.p45)
    m_params = list(m_spinham.p45)
...
def test_add(parameter1, parameter2):
    ...
    assert len(m_spinham.p45) == 96

    for i in range(2):
        assert np.allclose(
            m_spinham._45[i][-1], spinham1._45[i][-1] + spinham2._45[i][-1]
        )

    assert np.allclose(m_spinham._45[2][-1], spinham1._45[2][-1])
    assert np.allclose(m_spinham._45[3][-1], spinham2._45[2][-1])
...
def test_make_supercell(parameter1, i, j, k):
    ...
    assert len(new_spinham.p45) == i * j * k * len(spinham.p45)
