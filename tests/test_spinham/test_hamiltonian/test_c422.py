...
def test_add_422_when_present(when_present, parameter):
    spinham = get_spinham()

    spinham.add_422(alpha=0, beta=1, nu=(1, 0, 0), parameter=np.ones((3, 3, 3, 3)))
    ...
    assert np.allclose(spinham._422[0][3], parameter * np.ones((3, 3, 3, 3)))
...
def test_add_422_sorting(
    ...
    spinham.add_422(alpha3, beta3, nu3, parameter, when_present="replace")
    spinham.add_422(alpha4, beta4, nu4, parameter, when_present="replace")
