...
def test_add_21_when_present(when_present, parameter):
    spinham = get_spinham()

    spinham.add_21(0, np.ones((3, 3)))

    with pytest.raises(ValueError):
        spinham.add_21(0, np.ones((3, 3)))

    spinham.add_21(0, 2.0 * np.ones((3, 3)), when_present=when_present)
    assert np.allclose(spinham.p21[0][2], parameter * np.ones((3, 3)))
...
def test_add_21_sorting(alpha1, alpha2, alpha3, alpha4, parameter):
    ...
    spinham.add_21(alpha3, parameter, when_present="replace")
    spinham.add_21(alpha4, parameter, when_present="replace")

    for i in range(len(spinham.p21) - 1):
        assert spinham.p21[i][:2] <= spinham.p21[i + 1][:2]


@given(st.integers())
def test_remove_21(r_alpha):
    spinham = get_spinham()
    r_specs = (((0, 0, 0),), (r_alpha, r_alpha))

    for alpha in range(len(spinham.atoms.names)):
        spinham.add_21(alpha, np.ones((3, 3)))

    if 0 <= r_alpha < len(spinham.atoms.names):
        specs_before = [_[:-1] for _ in spinham.p21]
        spinham.remove_21(alpha=r_alpha)
        updated_specs = [_[:-1] for _ in spinham.p21]
...
def test_mul(parameter, number):
    ...
    for i in range(len(spinham.p21)):
        assert spinham.p21[i][0] == m_spinham.p21[i][0]
        assert spinham.p21[i][1] == m_spinham.p21[i][1]

        assert np.allclose(number * spinham.p21[i][2], m_spinham.p21[i][2])
...
def test_add(parameter1, parameter2):
    ...
    for i in range(2):
        assert np.allclose(m_spinham.p21[i][2], spinham1.p21[i][2] + spinham2.p21[i][2])

    assert np.allclose(m_spinham.p21[2][2], spinham1.p21[2][2])
