...
* Support ``--spin-values`` for the |GROGU|_ as well in ``magnopy-optimize-sd`` and
  ``magnopy-lswt``.
...
* Overall improvement of the console output for both ``magnopy-lswt`` and
  ``magnopy-optimize-sd``.
